// Улучшенное обучение нейросети на реалистичных изображениях цифр (20x30) - исправленная версия
...
const createImprovedDigitPatterns = () => {
  ...
  for (let digit = 0; digit <= 9; digit++) {
    ...
    for (let variation = 0; variation < variationsPerDigit; variation++) {
      ...
      const offsetX = Math.floor((Math.random() - 0.5) * 2); // Целочисленные сдвиги
      const offsetY = Math.floor((Math.random() - 0.5) * 2);
      
      switch(digit) {
        case 0: // Окружность
          const centerX = Math.floor(width / 2) + offsetX;
          const centerY = Math.floor(height / 2) + offsetY;
        ...
        case 1: // Вертикальная линия
          ...
          for (let y = Math.floor(height * 0.15); y < Math.floor(height * 0.85); y++) {
            if (midX >= 0 && midX < width) field[y][midX] = 1;
          }
        ...
        case 2: // Форма двойки
          ...
          for (let x = base2; x < right2; x++) {
            if (x >= 0 && x < width && top2 >= 0 && top2 < height) field[top2][x] = 1;
          }
          // Правая вертикальная (верхняя часть)
          for (let y = top2; y < mid2; y++) {
            if (y >= 0 && y < height && right2 >= 0 && right2 < width) field[y][right2] = 1;
          }
          // Средняя горизонтальная
          for (let x = base2; x < right2; x++) {
            if (x >= 0 && x < width && mid2 >= 0 && mid2 < height) field[mid2][x] = 1;
          }
          // Левая вертикальная (нижняя часть)
          for (let y = mid2; y < bottom2; y++) {
            if (y >= 0 && y < height && base2 >= 0 && base2 < width) field[y][base2] = 1;
          }
          // Нижняя горизонтальная
          for (let x = base2; x < right2; x++) {
            if (x >= 0 && x < width && bottom2 >= 0 && bottom2 < height) field[bottom2][x] = 1;
          }
          break;
        ...
        case 3: // Форма тройки
          ...
          for (let x = base3; x < right3; x++) {
            if (x >= 0 && x < width && top3 >= 0 && top3 < height) field[top3][x] = 1;
          }
          // Правая вертикальная
          for (let y = top3; y < bottom3; y++) {
            if (y >= 0 && y < height && right3 >= 0 && right3 < width) field[y][right3] = 1;
          }
          // Средняя горизонтальная
          for (let x = base3; x < right3; x++) {
            if (x >= 0 && x < width && mid3 >= 0 && mid3 < height) field[mid3][x] = 1;
          }
          // Нижняя горизонтальная
          for (let x = base3; x < right3; x++) {
            if (x >= 0 && x < width && bottom3 >= 0 && bottom3 < height) field[bottom3][x] = 1;
          }
          break;
        ...
        case 4: // Форма четверки
          ...
          for (let y = Math.floor(height * 0.2); y < mid4; y++) {
            if (y >= 0 && y < height && base4 >= 0 && base4 < width) field[y][base4] = 1;
          }
          // Средняя горизонтальная
          for (let x = base4; x < right4; x++) {
            if (x >= 0 && x < width && mid4 >= 0 && mid4 < height) field[mid4][x] = 1;
          }
          // Правая вертикальная
          for (let y = Math.floor(height * 0.2); y < Math.floor(height * 0.8); y++) {
            if (y >= 0 && y < height && right4 >= 0 && right4 < width) field[y][right4] = 1;
          }
          break;
        ...
        case 5: // Форма пятерки
          ...
          for (let x = base5; x < right5; x++) {
            if (x >= 0 && x < width && top5 >= 0 && top5 < height) field[top5][x] = 1;
          }
          // Левая вертикальная (верхняя часть)
          for (let y = top5; y < mid5; y++) {
            if (y >= 0 && y < height && base5 >= 0 && base5 < width) field[y][base5] = 1;
          }
          // Средняя горизонтальная
          for (let x = base5; x < right5; x++) {
            if (x >= 0 && x < width && mid5 >= 0 && mid5 < height) field[mid5][x] = 1;
          }
          // Правая вертикальная (нижняя часть)
          for (let y = mid5; y < bottom5; y++) {
            if (y >= 0 && y < height && right5 >= 0 && right5 < width) field[y][right5] = 1;
          }
          // Нижняя горизонтальная
          for (let x = base5; x < right5; x++) {
            if (x >= 0 && x < width && bottom5 >= 0 && bottom5 < height) field[bottom5][x] = 1;
          }
          break;
        ...
        case 6: // Форма шестерки
          ...
          for (let x = base6; x < right6; x++) {
            if (x >= 0 && x < width && top6 >= 0 && top6 < height) field[top6][x] = 1;
          }
          // Левая вертикальная
          for (let y = top6; y < bottom6; y++) {
            if (y >= 0 && y < height && base6 >= 0 && base6 < width) field[y][base6] = 1;
          }
          // Средняя горизонтальная
          for (let x = base6; x < right6; x++) {
            if (x >= 0 && x < width && mid6 >= 0 && mid6 < height) field[mid6][x] = 1;
          }
          // Правая вертикальная (нижняя часть)
          for (let y = mid6; y < bottom6; y++) {
            if (y >= 0 && y < height && right6 >= 0 && right6 < width) field[y][right6] = 1;
          }
          // Нижняя горизонтальная
          for (let x = base6; x < right6; x++) {
            if (x >= 0 && x < width && bottom6 >= 0 && bottom6 < height) field[bottom6][x] = 1;
          }
          break;
        ...
        case 7: // Форма семерки
          ...
          for (let x = base7; x < right7; x++) {
            if (x >= 0 && x < width && top7 >= 0 && top7 < height) field[top7][x] = 1;
          }
          // Правая вертикальная
          for (let y = top7; y < Math.floor(height * 0.8); y++) {
            if (y >= 0 && y < height && right7 >= 0 && right7 < width) field[y][right7] = 1;
          }
          break;
        ...
        case 8: // Форма восьмерки
          ...
          for (let x = base8; x < right8; x++) {
            if (x >= 0 && x < width && top8 >= 0 && top8 < height) field[top8][x] = 1;
          }
          // Средняя горизонтальная
          for (let x = base8; x < right8; x++) {
            if (x >= 0 && x < width && mid8 >= 0 && mid8 < height) field[mid8][x] = 1;
          }
          // Нижняя горизонтальная
          for (let x = base8; x < right8; x++) {
            if (x >= 0 && x < width && bottom8 >= 0 && bottom8 < height) field[bottom8][x] = 1;
          }
          // Левая вертикальная
          for (let y = top8; y < bottom8; y++) {
            if (y >= 0 && y < height && base8 >= 0 && base8 < width) field[y][base8] = 1;
          }
          // Правая вертикальная
          for (let y = top8; y < bottom8; y++) {
            if (y >= 0 && y < height && right8 >= 0 && right8 < width) field[y][right8] = 1;
          }
          break;
        ...
        case 9: // Форма девятки
          ...
          for (let x = base9; x < right9; x++) {
            if (x >= 0 && x < width && top9 >= 0 && top9 < height) field[top9][x] = 1;
          }
          // Средняя горизонтальная
          for (let x = base9; x < right9; x++) {
            if (x >= 0 && x < width && mid9 >= 0 && mid9 < height) field[mid9][x] = 1;
          }
          // Нижняя горизонтальная
          for (let x = base9; x < right9; x++) {
            if (x >= 0 && x < width && bottom9 >= 0 && bottom9 < height) field[bottom9][x] = 1;
          }
          // Левая вертикальная (верхняя часть)
          for (let y = top9; y < mid9; y++) {
            if (y >= 0 && y < height && base9 >= 0 && base9 < width) field[y][base9] = 1;
          }
          // Правая вертикальная
          for (let y = top9; y < bottom9; y++) {
            if (y >= 0 && y < height && right9 >= 0 && right9 < width) field[y][right9] = 1;
          }
          break;
      }
